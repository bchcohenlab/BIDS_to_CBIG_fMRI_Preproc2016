#!/bin/bash
# Script to run the CBIG Preproc2016 pipeline for a single subject in BIDS format on E2
...
if [ -f "${BIDSPATH}/CBIG_config.sh" ]; then
	...
	source /lab-share/Neuro-Cohen-e2/Public/repos/BIDS_to_CBIG_fMRI_Preproc2016/CBIG_E2_config.sh
fi
...
elif [ -f "${bids_dir}/${participant_id}/ses-baselineYear1Arm1/func/*task-rest*_bold.json" ]; then
	jq .'SliceTiming'[] `ls ${bids_dir}/${participant_id}/ses-baselineYear1Arm1/func/*task-rest*_bold.json | head -1` > $st_file
	echo "Used ${bids_dir}/${participant_id}/ses-baselineYear1Arm1/func/task-rest_bold.json for slice timing information"
